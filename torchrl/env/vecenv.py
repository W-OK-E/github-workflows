...
class VecEnv(BaseWrapper):
  ...
  def reset(self, **kwargs):
    # gymnasium reset() returns (obs, info) — extract obs
    obs = [env.reset(**kwargs)[0] for env in self.envs]
  ...
  def partial_reset(self, index_mask, **kwargs):
    ...
    reset_obs = [self.envs[index].reset(**kwargs)[0] for index in indexs]
  ...
  def step(self, actions):
    ...
    # gymnasium step() returns (obs, rew, terminated, truncated, info)
    result = [env.step(np.squeeze(action)) for env, action in
              zip(self.envs, actions)]
    obs, rews, terminateds, truncateds, infos = zip(*result)
    self._obs = np.stack(obs)
    terminated = np.stack(terminateds)[:, np.newaxis]
    truncated  = np.stack(truncateds)[:, np.newaxis]
